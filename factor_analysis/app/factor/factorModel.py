...
class factorModel:
    ...
    def getData(self):
        ...
        def get_val(bt_tradedate:list, hold_period:dict, pre_bt_tradedate:list, 
            ...
            value = SetUniverse(universe_index, pre_day)
            print(value)
            ...
            stock_daily_profit = GetHoldPeriodPriceRet(hold_datelst, df_f2, stk_api) #daily profit
        ...
        def run():
            ...
            #     p.join()
            length = len(self.bt_tradedate)
            # with Pool(length) as p:
            #     p.starmap(get_val, [(self.bt_tradedate, self.hold_period, self.pre_bt_tradedate, self.universe, self.universe_index,
            #                          self.factor_name_lst, i, all_period_data, self.stkapi, self.factor_api, delete_list) for i in range(length)])
    ...
    def calcBasketReturn(self, equityBasket:list, datesList:list, dailyReturns:list):
        ...
        for i in range(len(datesList)):
            ...
            dailyEquityReturn = dailyReturns.loc[datesList[i]:datesList[i], equityBasket]

            returnVal = np.dot(basketWeights, dailyEquityReturn.T).tolist()[0]
            basketReturns.append(returnVal)

            basketValue.append(basketValue[-1] * (1 + returnVal))
    ...
    def run(self):
        ...
        for i in range(len(equityGroups)):
            dailyReturn, AggValue = self.calcBasketReturn(equityGroups[i], dates, dailyReturns)
            groupedReturn.append(dailyReturn)
            groupedAggValue.append(AggValue)
