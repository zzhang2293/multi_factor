...
class factorModelSingleFactor(factorModel):
        ...
        for month in range(0, len(month_list)):
            ...
            for name in stock_names:
                try:
                    ...
                    pass
                    #print(f'{name} NOT AVAILABLE for {[month_list[month]]}')
    ...
    def run(self):
        ...
        _, Monthly_Equity_Returns, _, Equity_Idx_Monthly_Factor_Score, Daily_Equity_Returns, benchmark_dailyret = self.getData()
        
        df = pd.read_csv("factor.csv",header=None)
        factor_name_list = list(df.iloc[:,0])

        month_list = list(Monthly_Equity_Returns.keys())
